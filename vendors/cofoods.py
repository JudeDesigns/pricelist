"""
Cofoods Inc Parser
Handles PDF price lists from Cofoods Inc
"""

import re
from .base_parser import BaseParser

class CofoodsParser(BaseParser):
    def __init__(self):
        super().__init__()
        self.vendor_name = "Cofoods Inc"
        self.vendor_code = "cofoods"
    ...
    def validate_product_id(self, product_id):
        """Validate product ID format - flexible alphanumeric, but NOT size codes like 18/20"""
        if not product_id or product_id == "N/A":
            return True

        # Reject patterns like "18/20" or "19/119" (numbers with slashes - these are size codes, not IDs)
        if re.match(r'^\d+/\d+$', product_id):
            return False

        # Allow alphanumeric with hyphens, 3-15 characters (but no slashes)

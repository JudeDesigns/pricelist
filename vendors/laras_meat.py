"""
Laras Meat Vendor Parser
Handles Laras Meat specific product ID and pricing formats
"""
...
class LarasMeatParser(BaseParser):
    """Parser for Laras Meat price lists"""
    
    def __init__(self):
        super().__init__()
        self.vendor_name = "Laras Meat"
        self.vendor_code = "laras_meat"
    ...
    def extract_product_id(self, text):
        ...
        Laras Meat format: Flexible - can be numeric or alphanumeric
    ...
    def validate_product_id(self, product_id):
        """
        Validate Laras Meat product ID.
    ...
    def clean_cost(self, cost_str):
        """
        Clean and format cost string for Laras Meat.
    ...
    def parse_table_row(self, row):
        """
        Parse a table row from Laras Meat PDF.
        ...
        print(f"  DEBUG [Laras Meat Table]: Row has {len(row_clean)} columns: {row_clean}")
        ...
        if not description or len(description) < 2:
            print(f"  DEBUG [Laras Meat Table]: No valid description")
            return None
        
        # Skip if no price
        if not raw_price or not re.search(r'\d', raw_price):
            print(f"  DEBUG [Laras Meat Table]: No price found: '{raw_price}'")
            return None
        
        print(f"  DEBUG [Laras Meat Table]: Raw ID: '{raw_id}', Desc: '{description}', Price: '{raw_price}'")
        
        # Extract and validate product ID (or use N/A if no ID)
        if not raw_id or not raw_id.strip():
            product_id = 'N/A'
            print(f"  DEBUG [Laras Meat Table]: No product ID, using N/A")
        else:
            ...
            if not product_id or not self.validate_product_id(product_id):
                ...
                product_id = 'N/A'
                print(f"  DEBUG [Laras Meat Table]: Invalid product ID '{raw_id}', using N/A")
        
        print(f"  DEBUG [Laras Meat Table]: Valid product ID: '{product_id}'")
        
        # Clean the cost
        cost = self.clean_cost(raw_price)
        if not cost:
            print(f"  DEBUG [Laras Meat Table]: Invalid cost: '{raw_price}'")
            return None
        
        print(f"  DEBUG [Laras Meat Table]: ✅ SUCCESS - ID: {product_id}, Desc: {description}, Cost: {cost}")
    ...
    def parse_text_line(self, line):
        """
        Parse a text line from Laras Meat PDF (OCR-based).
        ...
        print(f"  DEBUG [Laras Meat]: Parsing line: '{line}'")
        ...
        if match:
            ...
            if self.validate_product_id(product_id) and self.is_valid_description(description):
                ...
                if cost_clean:
                    print(f"  DEBUG [Laras Meat]: ✅ Matched! ID={product_id}, Desc={description}, Cost={cost_clean}")
                    return {
                        'product_id': product_id,
        ...
        if match:
            ...
            if self.is_valid_description(description):
                cost_clean = self.clean_cost(cost)
                if cost_clean:
                    print(f"  DEBUG [Laras Meat]: ✅ Matched! ID=N/A, Desc={description}, Cost={cost_clean}")
                    return {
                        'product_id': 'N/A',
                        'description': description,
                        'cost': cost_clean
